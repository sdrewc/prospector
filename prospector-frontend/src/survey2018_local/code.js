...
const HH_VIEW = 'household_local';
const PERSON_VIEW = 'person_local';
const TRIP_VIEW = 'trip_local';
const LOCATION_VIEW = 'location_local';
...
const MODE_DETAIL_MAP = {1:'Walk',2:'Bike',3:'Bike',4:'Bike',
...
function getInfoHtml(geo) {
  ...
  if (geo.geometry.type == 'Point') {
    ...
    retval = '<b>HHID: </b>' + `${geo[HHID_VAR]}<br/>` +
                ...
                '<b><t>ALL MODES: </b>' + `${MODE_DETAIL_MAP[geo['mode_1']]}` + ',&nbsp' + `${MODE_DETAIL_MAP[geo['mode_2']]}` + ',&nbsp' + `${MODE_DETAIL_MAP[geo['mode_3']]}<br/>`
